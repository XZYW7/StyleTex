...
def make_predictor(feats_dim: object, output_dim: object, weight_norm: object = True) -> object:
    ...
    return module
@threestudio.register("styletex-mesh")
class StyleTexMesh(BaseExplicitGeometry):
    @dataclass
    ...
    def configure(self) -> None:
        ...
        if self.cfg.shape_init.startswith("mesh:"):
            ...
            self.std2mesh = std2mesh
            self.scale = scale / self.cfg.shape_init_params
    ...
    def mesh_to_init_shape(self,mesh) -> Mesh:
        def convert_data(data):
            if data is None:
                ...
                raise TypeError(
                    "Data must be in type numpy.ndarray, torch.Tensor, getting",
                    type(data),
                )
        v_pos = np.dot(self.std2mesh,convert_data(mesh.v_pos).T).T * self.scale + self.centroid
        v_nrm = np.dot(np.linalg.inv(self.std2mesh).T, convert_data(mesh.v_nrm).T).T

        v_pos = torch.tensor(v_pos, dtype=torch.float32).to(self.device).contiguous()
        t_pos_idx = torch.tensor(convert_data(mesh.t_pos_idx),dtype=torch.int64).to(self.device).contiguous()
        v_nrm = torch.tensor(np.ascontiguousarray(v_nrm), dtype=torch.float32).to(self.device)
        v_tex = torch.tensor(convert_data(mesh.v_tex), dtype=torch.float32).to(self.device).contiguous()
        return Mesh(v_pos=v_pos, t_pos_idx = t_pos_idx, v_nrm=v_nrm, v_tex=v_tex, t_tex_idx=t_pos_idx)
